...
public class InchMeterFabModuleController {
    ...
    // Existing CRUD operations (keeping all existing code)
    ...
    // NEW CLIENT-BASED ENDPOINTS - Similar to MTR Fab Module
    
    @GetMapping("/getDistinctClientNamesFromInchMeterFab/details")
    public ResponseEntity<List<String>> getDistinctClientNamesFromInchMeterFab() {
        LOG.info("Fetching distinct client names from Inch Meter Fab Module");
        try {
            List<String> clientNames = inchMeterFabModuleService.getDistinctClientNames();
    ...
    @GetMapping("/getDistinctWorkOrdersFromInchMeterFab/details")
    public ResponseEntity<List<String>> getDistinctWorkOrdersFromInchMeterFab(@RequestParam String clientName) {
        LOG.info("Fetching distinct work orders from Inch Meter Fab Module for client: {}", clientName);
        try {
            List<String> workOrders = inchMeterFabModuleService.getDistinctWorkOrdersByClientName(clientName);
    ...
    @GetMapping("/getDistinctServiceDescriptionsFromInchMeterFab/details")
    public ResponseEntity<List<String>> getDistinctServiceDescriptionsFromInchMeterFab(
            @RequestParam String clientName, 
            @RequestParam String workOrder) {
        LOG.info("Fetching distinct service descriptions from Inch Meter Fab Module for client: {} and work order: {}", clientName, workOrder);
        try {
            List<String> serviceDescriptions = inchMeterFabModuleService.getDistinctServiceDescriptionsByClientNameAndWorkOrder(clientName, workOrder);
    ...
    @GetMapping("/getDistinctRaNumbersFromInchMeterFab/details")
    public ResponseEntity<List<String>> getDistinctRaNumbersFromInchMeterFab(
            @RequestParam String clientName, 
            @RequestParam String workOrder, 
            @RequestParam String serviceDescription) {
        LOG.info("Fetching distinct RA numbers from Inch Meter Fab Module for client: {}, work order: {}, service description: {}", 
                clientName, workOrder, serviceDescription);
        try {
            List<String> raNumbers = inchMeterFabModuleService.getDistinctRaNumbersByClientNameAndWorkOrderAndServiceDescription(
                    clientName, workOrder, serviceDescription);
            LOG.info("Found {} distinct RA numbers", raNumbers.size());
            return ResponseEntity.ok(raNumbers);
        } catch (Exception e) {
    ...
    @GetMapping("/searchInchMeterFabData/details")
    public ResponseEntity<List<InchMeterFabModuleDto>> searchInchMeterFabData(
            @RequestParam String clientName,
            @RequestParam String workOrder,
            @RequestParam String serviceDescription,
            @RequestParam String raNumber) {
        LOG.info("Searching Inch Meter Fab data for client: {}, work order: {}, service description: {}, RA number: {}", 
                clientName, workOrder, serviceDescription, raNumber);
        try {
            List<InchMeterFabModuleDto> results = inchMeterFabModuleService.searchInchMeterFabDataByFilters(
                    clientName, workOrder, serviceDescription, raNumber);
            LOG.info("Found {} Inch Meter Fab records matching search criteria", results.size());
            return ResponseEntity.ok(results);
        } catch (Exception e) {
            LOG.error("Error searching Inch Meter Fab data: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
}
