...
public class BitsHeaderController {
    ...
    // Method to get work order details by direct work order number (URL-encoded)
    ...
    public ResponseEntity<?> getWorkOrderByBase64(@PathVariable String encodedWorkOrder) {
        try {
                    ...
                    try (ResultSet resultSet = statement.executeQuery()) {
                        if (resultSet.next()) {
                            ...
                            try {
                                ...
                                LOG.warn("Error parsing creation_date for work order {}", workOrder);
                            }
                            ...
                            try {
                                ...
                                LOG.warn("Error parsing last_update_date for work order {}", workOrder);
                            }
                                                    
                            try {
                                response.put("createdBy", resultSet.getLong("created_by"));
                            } catch (Exception ex) {
                                LOG.warn("Error parsing created_by for work order {}", workOrder);
                            }
                                                    
                            try {
                                response.put("lastUpdatedBy", resultSet.getLong("last_updated_by"));
                            } catch (Exception ex) {
                                LOG.warn("Error parsing last_updated_by for work order {}", workOrder);
                            }
            ...
            LOG.error("Error decoding Base64 work order or fetching details: {}", e.getMessage());
            return ResponseEntity.badRequest().body("Invalid Base64 encoding or error fetching data: " + e.getMessage());
        }
    }
 ...
 // ADDED: Implementing the endpoint for service lines by work order
    ...
    public ResponseEntity<List<Map<String, Object>>> getAllCustomers() {
        try {
            ...
            LOG.info("Successfully fetched {} customers from ledger_creation table", customers.size());
