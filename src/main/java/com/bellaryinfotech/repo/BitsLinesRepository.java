...
public interface BitsLinesRepository extends JpaRepository<BitsLinesAll, Long> {
    ...
    // NEW: Method to get the next line number for a work order
    ...
    // NEW: Method to get lines by work order number (joining with header table)
    ...
    // NEW: Method to count lines for a specific work order
    @Query("SELECT COUNT(l) FROM BitsLinesAll l WHERE l.orderId = :orderId")
    Long countByOrderId(@Param("orderId") Long orderId);
    
    // NEW: Method to validate line number uniqueness within a work order
