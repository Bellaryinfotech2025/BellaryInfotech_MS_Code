...
public interface RawMaterialEntryRepository extends JpaRepository<RawMaterialEntry, Long> {
    ...
    List<RawMaterialEntry> findByOrderId(Long orderId);
        ...
        // NEW: Find by sectionCode
    List<RawMaterialEntry> findBySectionCode(String sectionCode);

        List<RawMaterialEntry> findByCreatedBy(String createdBy);

        List<RawMaterialEntry> findByTenantId(Integer tenantId);

        // NEW: Find by receiptType
        List<RawMaterialEntry> findByReceiptType(String receiptType);
    ...
    List<String> findDistinctSections();
        ...
        // NEW: Find distinct section codes
    @Query("SELECT DISTINCT r.sectionCode FROM RawMaterialEntry r WHERE r.sectionCode IS NOT NULL")
    List<String> findDistinctSectionCodes();
